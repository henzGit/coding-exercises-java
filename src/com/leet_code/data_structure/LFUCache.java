...
class LFUCache {
    ...
    HashMap<Integer, LinkedHashSet<Integer>> freqs;//Counter and item list
    int cap;
    int min = -1;
    ...
    private void updateCount(int key) {
        ...
        if (count == min && freqs.get(count).size() == 0) min++;
        if (!freqs.containsKey(count + 1)) freqs.put(count + 1, new LinkedHashSet<>());
    ...
    public void put(int key, int value) {
        ...
        if (vals.size() >= cap) {
            int evit = freqs.get(min).iterator().next();
            freqs.get(min).remove(evit);
            vals.remove(evit);
            counts.remove(evit);
        }
